  ...
  return (
    <style>{`
      ...
      :root {
        ...
        --text-soft: #EAF0FF;
        --muted: #C7D0E6;
        --text-shadow-glow: 0 1px 3px rgba(0,0,0,0.55), 0 0 22px rgba(0,0,0,0.25);
        ...
        --glass-shadow: 0 8px 32px rgba(0,0,0,0.35), inset 0 1px 0 rgba(255,255,255,0.18);
        --glass-shadow-lg: 0 20px 60px rgba(0,0,0,0.45), inset 0 1px 0 rgba(255,255,255,0.2);
        --blur: blur(22px) saturate(180%);
        --blur-strong: blur(34px) saturate(190%);
      ...
      [data-theme="light"] {
        ...
        --text-soft: #1B1C28;
        --muted: #3F4358;
        --text-shadow-glow: 0 1px 0 rgba(255,255,255,0.6);
        ...
        --glass-shadow: 0 8px 28px rgba(60,50,110,0.16), inset 0 1px 0 rgba(255,255,255,0.7);
        --glass-shadow-lg: 0 20px 55px rgba(60,50,110,0.2), inset 0 1px 0 rgba(255,255,255,0.8);
      ...
      * { box-sizing: border-box; }
      .app {
        ...
        isolation: isolate;
        background:
          radial-gradient(1100px 700px at 12% -8%, var(--bg-a) 0%, transparent 60%),
          radial-gradient(1000px 800px at 100% 10%, var(--bg-b) 0%, transparent 55%),
          linear-gradient(160deg, var(--bg-c), var(--bg-a) 45%, var(--bg-b) 100%);
        min-height: 100vh;
        color: var(--text);
      ...
      /* Floating liquid blobs behind the glass */
      ...
      .blob-a { width: 480px; height: 480px; background: var(--blob1); top: -120px; left: -100px; animation: float-a 22s ease-in-out infinite; }
      .blob-b { width: 420px; height: 420px; background: var(--blob2); top: 30%; right: -120px; animation: float-b 26s ease-in-out infinite; }
      .blob-c { width: 460px; height: 460px; background: var(--blob3); bottom: -160px; left: 30%; animation: float-c 24s ease-in-out infinite; }
      @keyframes float-a { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(60px,50px) scale(1.08); } }
      @keyframes float-b { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(-50px,60px) scale(1.1); } }
      @keyframes float-c { 0%,100% { transform: translate(0,0) scale(1); } 50% { transform: translate(40px,-40px) scale(1.06); } }
      ...
      .theme-toggle:hover { transform: translateY(-1px) scale(1.03); background: var(--glass-bg); }
      ...
      .marquee-title h1 {
        ...
        letter-spacing: 4px;
        color: var(--amber);
        text-shadow: 0 0 24px rgba(255,197,61,0.55), var(--text-shadow-glow);
        margin: 0;
        font-weight: 800;
      }
      ...
      .gate-card, .stub, .mood-tile, .picker-dropdown, .picker-chosen, .picker-field,
      .settings-panel, .gate-form input, .empty {
      ...
      .gate-sub { color: var(--muted); margin: 6px 0 24px; font-size: 14px; font-weight: 600; text-shadow: var(--text-shadow-glow); }
      ...
      .gate-form input:focus { border-color: var(--amber); box-shadow: 0 0 0 3px rgba(255,197,61,0.25); }
      .gate-error { color: var(--danger); font-size: 13px; margin: 4px 0; font-weight: 700; text-shadow: var(--text-shadow-glow); }
      .gate-link { display: block; margin-top: 16px; font-size: 13px; color: var(--teal); text-decoration: none; font-weight: 700; text-shadow: var(--text-shadow-glow); }
      ...
      .onboard-sub { text-align: center; color: var(--muted); margin-bottom: 28px; font-weight: 600; text-shadow: var(--text-shadow-glow); }
      ...
      .picker-field:focus-within { border-color: var(--amber); box-shadow: 0 0 0 3px rgba(255,197,61,0.2); }
      ...
      .picker-option:hover { background: rgba(255,197,61,0.14); }
      .picker-option img { width: 30px; height: 45px; object-fit: cover; border-radius: 5px; }
      .picker-option-noimg { width: 30px; height: 45px; background: var(--glass-bg-soft); display: flex; align-items: center; justify-content: center; color: var(--muted); border-radius: 5px; }
      ...
      .picker-chosen { display: flex; align-items: center; gap: 10px; border-radius: 12px; padding: 7px 11px; }
      .picker-chosen img { width: 30px; height: 45px; object-fit: cover; border-radius: 5px; }
      ...
      .btn:hover:not(:disabled) { transform: translateY(-1px); background: var(--glass-bg-strong); }
      .btn:active:not(:disabled) { transform: translateY(0); }
      .btn:disabled { opacity: 0.5; cursor: not-allowed; }
      .btn-amber {
        background: linear-gradient(135deg, var(--amber-strong), var(--amber));
        color: #201400; border-color: rgba(255,255,255,0.5); font-weight: 800;
        text-shadow: none;
        box-shadow: 0 8px 26px rgba(255,197,61,0.35), inset 0 1px 0 rgba(255,255,255,0.5);
      }
      [data-theme="light"] .btn-amber { color: #241600; }
      .btn-amber:hover:not(:disabled) { background: linear-gradient(135deg, #FFE9A8, var(--amber-strong)); transform: translateY(-1px); }
      .btn-ghost { background: var(--glass-bg-soft); }
      .btn-ghost:hover { border-color: var(--amber-dim); }
      .btn-block { width: 100%; justify-content: center; margin-top: 6px; }
      .btn-icon {
        background: var(--glass-bg); border: 1px solid var(--glass-border-soft); color: var(--text); padding: 8px; border-radius: 999px; display: flex; align-items: center;
        backdrop-filter: var(--blur);
        -webkit-backdrop-filter: var(--blur);
      }
      .btn-icon:hover { color: var(--amber); background: var(--glass-bg-strong); }
      ...
      .brand { display: flex; align-items: center; gap: 8px; font-family: 'Bebas Neue', sans-serif; font-size: 20px; letter-spacing: 2px; color: var(--amber); text-shadow: var(--text-shadow-glow); }
      .tabs { display: flex; gap: 4px; flex: 1; flex-wrap: wrap; }
      .tab { background: transparent; border: 1px solid transparent; color: var(--muted); padding: 9px 14px; border-radius: 999px; font-size: 13px; font-weight: 700; text-shadow: var(--text-shadow-glow); }
      .tab.active {
        color: #201400; background: linear-gradient(135deg, var(--amber-strong), var(--amber));
        text-shadow: none; box-shadow: 0 6px 18px rgba(255,197,61,0.35);
      }
      [data-theme="light"] .tab.active { color: #241600; }
      .tab:not(.active):hover { color: var(--text); background: var(--glass-bg-soft); }
      .settings-panel { display: flex; gap: 10px; padding: 12px 4px 4px; flex-wrap: wrap; }

      .sprocket { display: flex; justify-content: space-between; padding: 0 2px; margin: 14px 0 18px; opacity: 0.6; }
      .sprocket-hole { width: 5px; height: 5px; border-radius: 50%; background: var(--muted); opacity: 0.6; }

      .tab-head { display: flex; align-items: flex-end; justify-content: space-between; gap: 12px; margin-bottom: 18px; flex-wrap: wrap; }
      .tab-head h2 { font-family: 'Bebas Neue', sans-serif; font-size: 26px; letter-spacing: 1px; margin: 0 0 2px; color: var(--text); text-shadow: var(--text-shadow-glow); font-weight: 800; }
      .tab-head p { color: var(--muted); font-size: 13px; margin: 0; font-weight: 600; text-shadow: var(--text-shadow-glow); }

      .loading { display: flex; align-items: center; gap: 8px; color: var(--text); font-weight: 700; padding: 30px 0; justify-content: center; font-size: 14px; text-shadow: var(--text-shadow-glow); }
      .loading.full { min-height: 100vh; }
      .empty { color: var(--text); font-weight: 700; text-align: center; padding: 40px 0; font-size: 14px; border-radius: 16px; border: 1px dashed var(--glass-border); text-shadow: var(--text-shadow-glow); }
      ...
      .stub:hover { transform: translateY(-3px); box-shadow: var(--glass-shadow-lg); }
      .stub-poster { position: relative; aspect-ratio: 2/3; background: var(--glass-bg-soft); }
      ...
      .stub .sprocket { margin: 0; padding: 8px 8px; background: var(--glass-bg-soft); border-bottom: 1px solid var(--glass-border-soft); opacity: 1; }
      .stub-body { padding: 13px 14px 15px; display: flex; flex-direction: column; gap: 8px; flex: 1; }
      .stub-title { font-size: 15px; font-weight: 800; margin: 0; line-height: 1.25; color: var(--text); text-shadow: var(--text-shadow-glow); }
      ...
      .mood-tile:hover { border-color: var(--teal); background: var(--glass-bg-strong); transform: translateY(-2px); }
      .mood-label { font-family: 'Bebas Neue', sans-serif; font-size: 20px; letter-spacing: 0.5px; color: var(--teal); text-shadow: var(--text-shadow-glow); font-weight: 800; }
